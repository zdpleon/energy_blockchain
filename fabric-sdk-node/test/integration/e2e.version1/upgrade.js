...
test('\n\n***** U P G R A D E flow: chaincode install *****\n\n', (t) => {
	e2eUtils.installChaincode('org1', testUtil.CHAINCODE_UPGRADE_PATH, 'v112', t)
	.then(() => {
		t.pass('Successfully installed chaincode in peers of organization "org1"');
		return e2eUtils.installChaincode('org2', testUtil.CHAINCODE_UPGRADE_PATH, 'v112', t);
	}, (err) => {
...
test('\n\n***** U P G R A D E flow: upgrade chaincode *****', (t) => {
	e2eUtils.instantiateChaincode('org1', testUtil.CHAINCODE_UPGRADE_PATH, 'v112', true, t)
	.then((result) => {
		if(result){
...
test('\n\n***** U P G R A D E flow: invoke transaction to move money *****', (t) => {
	e2eUtils.invokeChaincode('org2', 'v112', t)
	.then((result) => {
		if(result){
...
test('\n\n***** U P G R A D E flow: query chaincode *****', (t) => {
	e2eUtils.queryChaincode('org2', 'v112', '410', t)
	.then((result) => {
		if(result){
